<?php 
	...
	$querystring = "select * from posts where (posthash not like 'text-%') order by posttime desc limit 60;";
	$result = mysqli_query($link, $querystring);
	$postList = array();
	...
	$alreadyPostArray = array();
	$alreadyPostArray["someFillerBullshit"] = 'o';
	...
	for ($k=0;$k<count($postList);$k++)
	{
		...
		if (array_key_exists($posthash, $alreadyPostArray))
		{
			continue;
		}
		$alreadyPostArray[$posthash] = '420';
