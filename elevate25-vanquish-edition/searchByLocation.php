<?php
	...
	for ($i=(count($proximityArray)-1);$i>=0;$i--)
	{
		...
		if (!empty($aDojo))
		{
			...
			$querystring = "SELECT * FROM spiegelcount WHERE (dojohash='$dojoHash' and username='$username');";
			$result = mysqli_query($link, $querystring);
			$spiegelResults = array();
			while ($unique = mysqli_fetch_assoc($result)) {
				//grab the dojos name and set it
				$spiegelResults[] = $unique; 
			}
			...
			if (!empty($selectResults))
			{
				if ((abs($dojoLongi - $longi) < 0.015) && (abs($dojoLati - $lati) < 0.015))
				{
					...
					array_push($interim, $nojoResults);
					array_push($interim, $metric);
					array_push($interim, $spiegelResults);
