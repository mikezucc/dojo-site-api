<?php 
	...
	$querystring = "insert into nodes (nodehash, username, made, title) values ('$nodehash', '$username','$currdate', '$sessionName');";
	$result = mysqli_query($link, $querystring);
	...
	for ($i=0;$i<($numOfDojos-1);$i++)
	{
		...
		$result = mysqli_query($link, $querystring);
		$aDojoRosterDat = array();
		while($row = mysqli_fetch_assoc($result)) {
			$aDojoRosterDat[] = $row; 
		}
		for ($k=0;$k<count($aDojoRosterDat);$k++)
		{
			$aPerson = $aDojoRosterDat[$k]['username'];
			
			$querystring = "select * from spiegelcount where (dojohash='$dojohash' and username='$aPerson');";
			$result = mysqli_query($link, $querystring);
			$slackDat = array();
			while($row = mysqli_fetch_assoc($result)) {
				$slackDat[] = $row; 
			...
			if ( count($slackDat) > 0)
			{
				$querystring = "update spiegelcount set counter = counter + 1, updated='$currdate' where (dojohash='$dojohash' and username='$aPerson');";
				$result = mysqli_query($link, $querystring);
			}
			else
			{
				$querystring = "insert into spiegelcount (dojohash, username, counter, updated) values ('$dojohash', '$aPerson', 1, '$currdate');";
				$result = mysqli_query($link, $querystring);
			}
		}
