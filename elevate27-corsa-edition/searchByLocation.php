<?php
	...
	for ($i=(count($proximityArray)-1);$i>=0;$i--)
	{
		...
		if (!empty($aDojo))
		{
			...
			if (!empty($selectResults))
			{
				...
				{
					$metric = "campus";
					array_push($interim2, $aDojo);
					$approxDistance = sqrt((pow(abs($dojoLongi - $longi),2)) + (pow(abs($dojoLati - $lati),2)));
					array_push($interim2, $approxDistance);
					array_push($interim2, $rosterresults);
					array_push($interim2, $metric);
					array_push($interim2, $dojoName);
					array_push($interim2, $dojoHash);
					array_push($interim2, $postResults);
					array_push($interim2, $lastPost);
					array_push($interim2, $dojoLongi);
					array_push($interim2, $dojoLati);
					array_push($distanceClose, $interim2);
					continue;
				}
				if ((abs($dojoLongi - $longi) < 0.3) && (abs($dojoLati - $lati) < 0.3))
				{
					...
					array_push($interim2, $dojoLongi);
					array_push($interim2, $dojoLati);
					array_push($distance25, $interim2);
					continue;
				}
			}
		}
	...
	array_push($finalProximity, $areaMap);
