<?php 
	...
	if ($isRoster == 1)
	{
		$dojoInfo[0]['following'] = "yes";
	}
	else
	{
		$dojoInfo[0]['following'] = "no";
	}
	...
	for ($k=0;$k<count($postList);$k++)
	{
		...
		while($row = mysqli_fetch_assoc($result)) {
			$commentboard[] = $row;
			$posterhash = $row["username"];
			$querystring2 = "select * from users where (username='$posterhash');";
			...
			$commentboard[$ctr]["interval"] = $strang;
			$commentboard[$ctr]["fullname"] = $commenterInfo[0]["fullname"];
			$commentboard[$ctr]["profilehash"] = $commenterInfo[0]["profilehash"];
			$commentboard[$ctr]["glory"] = $commenterInfo[0]["glory"];
			$commentboard[$ctr]["bio"] = $commenterInfo[0]["bio"];
			$ctr++;
		}
		...
		$fuck = 0;
		if ((count($commentboard) - $repostInfo) - 1 > 0)
		{
			$fuck = (count($commentboard) - $repostInfo - 1);
		}
		else
		{
			$fuck = 0;
		...
		array_push($intermediate, $commentboard);
