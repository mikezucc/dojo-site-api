<?php 
	...
	$result = mysqli_query($link, $querystring);
	$initialDojoPost = array();
	while($row = mysqli_fetch_assoc($result)) {
		$initialDojoPost[] = $row; 
	}
	$initialdojohash = $initialDojoPost[0]['dojohash'];
	
	$querystring = "select * from elements where (nodehash='$nodehash');";
	$result = mysqli_query($link, $querystring);
	$elemAuthorDat = array();
	while($row = mysqli_fetch_assoc($result)) {
		$elemAuthorDat[] = $row; 
	}
	$foundTheAuthor = false;
	for ($k = 0; $k < count($elemAuthorDat); $k++)
	{
		$eachemail = $elemAuthorDat[$k]['username'];
		if ($eachemail  !== $username)
		{
			$querystring = "insert into noti420  (made, forwhom, instigator, type, payload, seen, target, subject) values ( '$currdate', '$eachemail', '$username', 'append', ' added something', 'no', '$initialdojohash', '$nodehash');";
			$result = mysqli_query($link, $querystring);
		}
	}
